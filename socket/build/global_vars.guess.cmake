...
#    - android: Navigation runs on Android (DDNInfo, etc.)
#    - iOS_DDN: Navigation runs on iOS (iDDN, etc.)
#    - (other): (to be added)
#
if (NOT DEFINED USER_CONF_TARGET_PRODUCT)
    if (NUTSHELL AND (DEFINED ENV{TARGET_PRODUCT}) AND (NOT "$ENV{TARGET_PRODUCT}" STREQUAL ""))
        set (USER_CONF_TARGET_PRODUCT "$ENV{TARGET_PRODUCT}")
    elseif (ANDROID)
        set (USER_CONF_TARGET_PRODUCT android)
    elseif (IOS)
        set (USER_CONF_TARGET_PRODUCT iOS_DDN)
    else ()
        set (USER_CONF_TARGET_PRODUCT Navi)
    endif ()
endif ()
...
if (NOT DEFINED USER_CONF_TARGET_DEVICE)
    if (NUTSHELL AND (DEFINED ENV{TARGETPRODUCT}) AND (NOT "$ENV{TARGETPRODUCT}" STREQUAL ""))
        set (USER_CONF_TARGET_DEVICE "$ENV{TARGETPRODUCT}")
    elseif (ANDROID)
...
#    - DCU: Display Control Unit (for 17cy only)
#    - MEU: Multimedia Extension Unit (for 17cy only)
#    - DUC-MEU: for all projects other than 17cy
#
if (NOT DEFINED USER_CONF_TARGET_BOARD_TYPE)
    # default to build project for both MEU & DCU
    if (NUTSHELL)
        if ("$ENV{TARGET_PRODUCT}" MATCHES "dcu_e2")
            set (USER_CONF_TARGET_BOARD_TYPE "DCU")
        elseif ("$ENV{TARGET_PRODUCT}" MATCHES "dcu_m2n")
            set (USER_CONF_TARGET_BOARD_TYPE "DCU")
        elseif ("$ENV{TARGET_PRODUCT}" MATCHES "generic_dcue2")
            set (USER_CONF_TARGET_BOARD_TYPE "DCU")
        elseif ("$ENV{TARGET_PRODUCT}" MATCHES "generic_dcum2n")
            set (USER_CONF_TARGET_BOARD_TYPE "DCU")
        elseif ("$ENV{TARGET_PRODUCT}" MATCHES "meu_m2w")
            set (USER_CONF_TARGET_BOARD_TYPE "MEU")
        elseif ("$ENV{TARGET_PRODUCT}" MATCHES "generic_meum2w")
            set (USER_CONF_TARGET_BOARD_TYPE "MEU")
        elseif ("$ENV{TARGET_PRODUCT}" MATCHES "generic_x86")
            set (USER_CONF_TARGET_BOARD_TYPE "MEU")
        else ()
            message(FATAL_ERROR "Unknown target product: \"$ENV{TARGET_PRODUCT}.\"")
        endif ()
    else (NUTSHELL)
        set (USER_CONF_TARGET_BOARD_TYPE "MEU")
    endif (NUTSHELL)
...
if (NOT DEFINED USER_CONF_USE_EGL_PLATFORM)
    # default to use EGL I/F same as android
    set (USER_CONF_USE_EGL_PLATFORM "android")
endif ()
...
if (NOT DEFINED USER_CONF_EXTERNAL_MODULES_OUTPUT_DIR)
    ...
    if (NUTSHELL)
        ...
        get_filename_component (USER_CONF_EXTERNAL_MODULES_OUTPUT_DIR "${USER_CONF_PROJECT_ROOT_DIR}/out" ABSOLUTE)
#        file (RELATIVE_PATH USER_CONF_TARGET_OUT_INTERMEDIATES "$ENV{PWD}" "${USER_CONF_EXTERNAL_MODULES_OUTPUT_DIR}")
    endif (NUTSHELL)
else()
endif ()
